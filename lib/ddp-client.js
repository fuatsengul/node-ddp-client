...
var DDPClient = function(opts) {
  ...
  self.idPrefix = opts.idPrefix || "";
  // Expose EJSON object, so client can use EJSON.addType(...)
  self.EJSON = EJSON;
  self.sharedSocketMode = opts.sharedSocketMode || false;
...
DDPClient.prototype._send = function(data) {
  if(this.sharedSocketMode){
    this.emit("socket-send", data);
  } else {
...
DDPClient.prototype._getNextId = function() {
  ...
  return self.idPrefix + (this._nextId += 1).toString();
};
 ...
 */
DDPClient.prototype.connect = function(reconnectCallback) {
  var self = this;
  self._reconnectCallback = self._reconnectCallback || reconnectCallback;
  ...
  request.get(requestOpts, function(err, res, body) {
    if (err) {
      ...
      try {
        ...
        info = {base_url : ''};
        console.log('=> Your Meteor application has errors.')
      }
